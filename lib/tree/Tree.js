  ...
  */
    this.getBranch = function(dir, ignored = []){
        ...
        fs.readdir(dirPath, (err, items) => {
            ...
            if(!(items === null || undefined)) this.findDirectories(dirPath, items);
...
/**
   ...
   */
    this.isMatch = function(fullFilePath){
        let fileExt = path.extname(fullFilePath);
        let dirname = path.dirname(fullFilePath)        
        this.extensions.forEach( ext => {
            if(fileExt === ext){
                this.files.push(fullFilePath);
                this.extensionsMatched[ext] = true;
                this.emit('file', fullFilePath, dirname, ext);
            } 
            return;
        });
        return;
